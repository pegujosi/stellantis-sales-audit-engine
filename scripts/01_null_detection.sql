--query shows string null quantity on sales column
SELECT COUNT(*) AS nulos
FROM fact_ventas
WHERE venta_bruta IS NULL;

--query shows string null quantity per column (no primary key)
SELECT
...
--query shows total string null quantity per column 
SELECT COUNT(*) - COUNT(*) FILTER (WHERE venta_bruta IS NOT NULL) AS nulos
FROM fact_ventas;

--query shows total string null quantity on sales column
